...
b8 platform_startup(
  ...
  if (!state->app_delegate) {
    B_ERROR("Failed to create application delegate")
    return FALSE;
  }
  ...
  if (!state->wnd_delegate) {
    B_ERROR("Failed to create window delegate")
    return FALSE;
  }
  ...
  state->window = [[NSWindow alloc]
    ...
    defer:NO];
  if (!state->window) {
    B_ERROR("Failed to create window");
    return FALSE;
  }
  ...
  if (!state->layer) {
    B_ERROR("Failed to create layer for view");
  }
